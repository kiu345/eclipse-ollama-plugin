...
public class ChatGPTViewPart
{
    ...
    private String loadCss()
    {
        ...
        {
            try (InputStream in = FileLocator.toFileURL( new URL( "platform:/plugin/com.github.kiu345.eclipse.plugin.assistai.main/css/" + file ) )
                    .openStream())
            {
    ...
    private String loadJavaScripts()
    {
        ...
        {
            try (InputStream in = FileLocator.toFileURL( new URL( "platform:/plugin/com.github.kiu345.eclipse.plugin.assistai.main/js/" + file ) )
                    .openStream())
            {
        ...
        public void add( ImageData preview, String caption )
        {
            ...
            try
            {
                imageDescriptor = Optional.ofNullable( preview ).map( id -> ImageDescriptor.createFromImageDataProvider( zoom -> id ) ).orElse(
                        ImageDescriptor.createFromURL( new URL( "platform:/plugin/com.github.kiu345.eclipse.plugin.assistai.main/icons/folder.png" ) ) );
            }
